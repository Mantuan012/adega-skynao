    ...
    <div>
      ...
      {pedidosFiltrados.map(
        ({
          ...
          // --- MUDANÇA 1: A lógica de 'agrupamento' foi removida ---
          // const agrupamento = itens.reduce(...) <-- REMOVIDO

          // --- MUDANÇA 2: O 'totalPedido' agora usa a nova lógica no fallback ---
          const totalPedido = total
            ? total
            : // Calcula o subtotal + frete (3.00) se o 'total' não foi salvo
              itens.reduce((acc, item) => acc + (item.preco * item.quantidade), 0) + 3;
          ...
          return (
            <div key={idDoc} className="cartao">
              ...
              <p>
                <b>Itens:</b>{" "}
                {/* --- MUDANÇA 3: Lendo o novo formato de 'itens' --- */}
